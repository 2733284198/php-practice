...
//评论消息处理
	function onMessage(evt) {
		...
		switch (type) {
			...
			case "say"://用户在直播间说话
				...
				break;
			case "notice"://直播间的通知
				$('#notice').find('.notice-view').text(obj.content);
				notice()
				break;
	...
	function setVal(data) {
		$("#CurrentPlay").text(data.currentPlay);
		$("#CurrentView").text(data.currentView);
		$("#TotalView").text(data.totalView);
		$("#TotalPlay").text(data.totalPlay);
	}
...
<!--频道相关-->
	...
	function onChannelPlay(channel) {
		...
		var state = ch.data("state");
		src1 = "http://pubstream.amailive.com/live/601.m3u8";
		src2 = "http://image.amailive.com/data/L01751/video/L01751_201610152110521476537601/index.m3u8";
	...
	function createCommentElementByOther(comment, insertTop, scroll) {
		...
		var li = '<li>' +
			...
			'</div>' +
			'<div class="dis_right">' +
			'<h3><span class="comment-username">小李子</span><span>(<i class="time">14:16:11</i><i class="trueTime" style="display: none;"></i>)</span></h3>' +
			'<p class="comment-detail">' +
			'在好课网可以免费创建和发布课程在好课网可以免费创建和发布课程在好课网可以免费创建和发布课程' +
			'</p>' +
			'</div>' +
