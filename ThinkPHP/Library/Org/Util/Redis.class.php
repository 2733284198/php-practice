...
class Redis
{
    ...
    private function initRedis($conf){
        ...
        foreach($conf['master'] as $value => $key){
            $objMaster = new \Redis();
            if($objMaster ->pconnect($value['host'],$value['port'])){
                $objMaster ->auth($value['auth']);
                self::$linkHandle['master'][] = $objMaster ;
            }
        }
        /** Slave */
        foreach($conf['slave'] as $v => $k){
            $objSlave = new \Redis();
            if($objSlave->pconnect($v['host'],$v['port'])){
                $objSlave->auth($v['auth']);
                self::$linkHandle['slave'][] = $objSlave;
            }
        }
    }
